...
- Harness health: automatically measurable repository evidence, such as durable
  agent instructions, runnable checks, drift scripts, source tracking, and
  complete adoption reports. Harness Doctor and local validation checks belong
  here.
...
template setup, placeholder-prompt, and workflow-preparation records can be
...
6. Store aggregate results in an effectiveness report using
   ...
   copy `docs/templates/task-outcome.yaml` and store filled records under
   `docs/effectiveness/task-outcomes/`.
7. For each task outcome record, include the repository ref, prompt reference,
   run id, reviewer, harness source, and verification command so later
   reviewers can tell whether two runs are actually comparable.
...
The current `today-bus` evaluation mode is harnessed-only tracking unless a
future reviewer provides comparable baseline data. A single task outcome record
must not be reported as proof that the harness improved agent effectiveness.

Before publishing or reusing TodayBus dogfood evidence, use
`docs/checklists/dogfood-evidence-adoption.md` to verify source tracking, task
outcomes, failure memory, gate placement, and claim boundaries.
